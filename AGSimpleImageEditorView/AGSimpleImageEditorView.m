...
@synthesize overlayPanGestureRecognizer = _overlayPanGestureRecognizer;
...
{
    if (_overlayPanGestureRecognizer != nil)
    {
        return _overlayPanGestureRecognizer;
    }
    _overlayPanGestureRecognizer = [[UIPanGestureRecognizer alloc] initWithTarget:self action:@selector(overlayPanGesture:)];
    return _overlayPanGestureRecognizer;
...
- (void)dealloc
{
    ...
    [_overlayPanGestureRecognizer release];
...
- (void)initialize
{
    ...
    [overlayView addGestureRecognizer:self.overlayPanGestureRecognizer];
    ...
    ratioView.autoresizesSubviews = YES;
    ratioView.userInteractionEnabled = YES;
...
//    self.textView.frame = ratioView.frame;
//    self.textView.textColor = [UIColor whiteColor];
//    self.textView.textAlignment = UITextAlignmentCenter;
//    self.textView.backgroundColor = [UIColor colorWithWhite:1 alpha:0.05f];
//    self.textView.font = [UIFont systemFontOfSize:20];
//    [ratioView addSubview:self.textView];
...
- (void)panGesture:(UIPanGestureRecognizer *)recognizer
{
    ...
    [recognizer setTranslation:CGPointMake(0, 0) inView:self.ratioControlsView];
...
- (void)overlayPanGesture:(UIPanGestureRecognizer *)recognizer
{
    CGPoint translation = [recognizer translationInView:self.overlayView];
    CGPoint tapLocation = [recognizer locationInView:self];
    CGRect newRect = self.cropRect;
    //left drag
    if (tapLocation.x < self.cropRect.origin.x)
    {
        newRect = CGRectMake(self.cropRect.origin.x + translation.x, self.cropRect.origin.y, self.cropRect.size.width - translation.x, self.cropRect.size.height);

    }
    //top drag
    else if (tapLocation.y < self.cropRect.origin.y)
    {
        newRect = CGRectMake(self.cropRect.origin.x, self.cropRect.origin.y + translation.y, self.cropRect.size.width, self.cropRect.size.height - translation.y);
    }
    //right drag
    else if (tapLocation.x > (self.cropRect.origin.x + self.cropRect.size.width))
    {
        newRect = CGRectMake(self.cropRect.origin.x, self.cropRect.origin.y, self.cropRect.size.width + translation.x, self.cropRect.size.height);
    }
    //bottom drag
    else if (tapLocation.y > (self.cropRect.origin.y + self.cropRect.size.height))
    {
        newRect = CGRectMake(self.cropRect.origin.x, self.cropRect.origin.y, self.cropRect.size.width, self.cropRect.size.height + translation.y);
    }
    else
    {}
    ...
    [recognizer setTranslation:CGPointMake(0, 0) inView:self.overlayView];
